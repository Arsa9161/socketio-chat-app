...
function App() {
  ...
  // chat-дахь мессежүүдийн жагсаалт
  ...
  useEffect(() => {
    // server-тэй холбогдох
    socket = io("http://localhost:5000");
    // message ирэх үед жагсаалт руу нэмнэ
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // чатны мессежийг server-рүү илгээнэ
  ...
  const Message = ({ messageId, name, message }) => {
    ...
    return (
      <li className={isMe ? "text-right" : "text-left"}>
        ...
        <p
          className={`py-1 px-4 mb-3 rounded-2xl inline-block ${
            isMe ? "bg-gray-200" : "bg-yellow-500 text-white"
          }`}
        >
          {message}
        </p>
  ...
  // MessagesContainer мессежүүдийг агуулна
  ...
  const Textarea = () => (
    <form className="max-h-32 py-3 px-5 flex justify-around align-center">
      <input
        ...
        type="text"
        placeholder="Aa"
        className="bg-gray-200 rounded-2xl h-10 outline-none px-3 py-1"
      />
      <button
        onClick={handleSubmit}
        className="w-10 h-10 rounded-full text-yellow-500 p-1.5 outline-none hover:text-yellow-600"
      >
        {sendIcon}
      </button>
    </form>
